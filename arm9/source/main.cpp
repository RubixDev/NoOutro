...
MenuItem createSubMenu(std::string title, std::string ident, std::string dir, std::string remote_dir, PrintConsole *topConsole, PrintConsole *bottomConsole) {
    return {title, 0, [topConsole, bottomConsole, title, ident, dir, remote_dir](Menu &menu) {
        ...
        Menu subMenu(title);
        subMenu.addItem({"Browse Titles                X", KEY_X, [topConsole, bottomConsole, html_path, roms_path, url](Menu &menu) {
            consoleClear();
            Menu::print("Reading index file...\n");
            const char *html_c = read_file(html_path.c_str());
            if (!html_c) {
                Menu::prompt(KEY_A | KEY_B, "Could not read index file!\nPlease make sure you have\ndownloaded the latest index\nusing the \"Update Index\" menu\noption.\n");
                return;
            }
