...
std::string kbdGetString(std::string label, int maxSize, std::string oldStr) {
    ...
    while (!done) {
        ...
        Menu::print(
            ">%c%-29.29s%c\n",
            (scrollPosition > 0) ? '\x1B' : ' ',
            output.c_str() + scrollPosition,
            (scrollPosition < (int)output.size() - (SCREEN_COLS - 3)) ? '\x1A' : ' '
        );
        ...
        Menu::print(
            "\x1B[42m\x1B[2;%dH%s\x1B[47m\n\n",
            2 + cursorPosition,
            stringPosition < (int)output.size() ? output.substr(stringPosition, charLen).c_str()
                                                : "_"
        );
        ...
        default: // Letter
            if (output.size() < (uint)maxSize) {
                ...
                if (cursorPosition + 1 >= (SCREEN_COLS - 3) &&
                    stringPosition <= (int)output.size()) {
                    scrollPosition++;
                    while ((output[scrollPosition] & 0xC0) == 0x80) // UTF-8
                        scrollPosition++;
                }
